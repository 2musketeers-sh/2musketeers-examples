...
function docker_run () {
  ...
  local DOCKER_GID=$(getent group docker | cut -d: -f3)
    ...
    # NOTE: Output is only shown on error, OR if it's taking longer than 3 seconds to complete the build
    #
    ...
    (docker build -t "${CONTAINER_PREFIX}_2musketeers:latest" \
      ...
      -f "${DOCKER_BUILD_FILE}" .  &>>.docker-build-output || \
      { ERRCODE=$?; kill $loggerpid ; cat .docker-build-output ; exit ${ERRCODE}; }) & #show output on failure
    dockerbuild_pid=$!
    ( sleep 3; tail --pid=$dockerbuild_pid --lines=+0 -f .docker-build-output ) & #wait 5 secs then start showing the docker build output
    loggerpid=$!
